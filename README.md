# Projet @bricobus

# Projet Arduino : [test_dht11](./test_dht11/dht_test/)

# Schema : [DHT11](./Schema/dht11.qet)

# Serié : [Documentation](./Serie/doc.md)

## Liens :
* [[Tuto] Utiliser un capteur température humidité](https://letmeknow.fr/blog/2013/11/06/tuto-utiliser-un-capteur-temperature-humidite/)
* [Utiliser un capteur de température et d'humidité DHT11 / DHT22 avec une carte Arduino / Genuino](https://www.carnetdumaker.net/articles/utiliser-un-capteur-de-temperature-et-dhumidite-dht11-dht22-avec-une-carte-arduino-genuino/)
* [Arduino library for DHT11DHT22, etc Temp & Humidity Sensors](https://github.com/adafruit/DHT-sensor-library)
* [Common sensor library](https://github.com/adafruit/Adafruit_Sensor)
* [Adafruit GFX graphics core library](https://github.com/adafruit/Adafruit-GFX-Library)
* [Arduino library and example code for the 16x32 RGB matrix panels in the shop](https://github.com/adafruit/RGB-matrix-Panel)
* [Adafruit 32x16 and 32x32 RGB LED Matrix](https://learn.adafruit.com/32x16-32x32-rgb-led-matrix/test-example-code)
* [37-SENSOR-KIT-TUTORIAL-FOR-UNO-AND-MEGA](http://mthackathon.info/resources/37-SENSOR-KIT-TUTORIAL-FOR-UNO-AND-MEGA.pdf)
* [Le port série du Raspberry Pi 3 : pas simple !](https://www.framboise314.fr/le-port-serie-du-raspberry-pi-3-pas-simple/)
* [Raspberry Pi – Arduino – Lier les deux via Serie / UART](http://www.pihomeserver.fr/2015/01/23/raspberry-pi-home-server-arduino-lier-les-deux-via-serieuart/)